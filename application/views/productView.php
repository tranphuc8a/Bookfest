...
<body ng-app="product" ng-controller="productView" class="background-body" ng-init="init()">
			...
			<div class="card border-0 rounded background-body px-0" style="width: min(90%, max(60%, 600px));">
				<div class="card-header background-body border-0 p-0">
					<div class="nav nav-tabs border-0 p-0">
						<a href="#detail" ng-click="change(0)" class="nav-link border-0 {{active[0]}}">{{book.name}}</a>
						<a href="#comment" ng-click="change(1)" class="nav-link border-0 {{active[1]}}">Đánh giá</a>
					</div>
				</div>
	...
	<script type="text/javascript" src="<?php echo base_url();?>/angular/javascript/product.js"></script>
